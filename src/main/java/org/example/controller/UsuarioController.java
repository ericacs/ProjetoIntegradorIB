...
public class UsuarioController {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (Pattern.matches("/usuarios/\\d+", path)) {
    ...
    private void handleUpdateRequest(HttpExchange exchange) throws IOException {
        try {
            ...
            Usuario usuario = om.readValue(inputStream, Usuario.class);

            Usuario usuarioSaved = dao.getPorId(usuario.getIdUsuario());

            String response;
            if (usuarioSaved != null) {
                usuarioSaved.setContato(usuario.getContato());
                usuarioSaved.setNome(usuario.getNome());
                usuarioSaved.setEndereco(usuario.getEndereco());
                dao.atualizar(usuarioSaved);
                ...
                response = "Usuário não econtrado!";
    ...
    private void handleGetRequest(HttpExchange exchange) throws IOException {
        try {
            ...
            String response = om.writeValueAsString(usuarios);
    ...
    private void handlePostRequest(HttpExchange exchange) throws IOException {
        try {
            ...
            Usuario usuario = om.readValue(inputStream, Usuario.class);
            dao.salvar(usuario);
            String response = om.writeValueAsString(usuario);
            exchange.getResponseHeaders().set("Content-Type", "application/json");
            exchange.sendResponseHeaders(200, response.getBytes().length);
            OutputStream os = exchange.getResponseBody();
            os.write(response.getBytes());
